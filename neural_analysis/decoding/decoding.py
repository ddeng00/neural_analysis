...
from ._decoding import (
    ...
    _decode_cross_cond_ref_to_gen_helper,
    _decode_cross_time_helper,
    _decode_helper,
    _decode_ref_to_gen_helper,
)
...
def decode_cross_cond_ref_to_gen(
    ...
    condition_col: str,
    neuron_col: str,
    min_trials_ref: int | None = None,
    min_trials_gen: int | None = None,
    n_samples: int = 250,
    subsample_ratio: float = 1.0,
    n_permute: int = 10,
    same_cond_only: bool = False,
    ...
    for col in spike_rate_cols:
        if col not in data_ref.columns:
            raise ValueError(f"Column '{col}' not found in reference data.")
        if col not in data_gen.columns:
            raise ValueError(f"Column '{col}' not found in generalize data.")
    if variable_col not in data_ref.columns:
        raise ValueError(f"Column '{variable_col}' not found in reference data.")
    if variable_col not in data_gen.columns:
        raise ValueError(f"Column '{variable_col}' not found in generalize data.")
    if set(data_ref[variable_col].unique()) != set(data_gen[variable_col].unique()):
        raise ValueError(
            "Reference and generalize data must have the same variable values."
        )
    if neuron_col not in data_ref.columns:
        raise ValueError(f"Column '{neuron_col}' not found in reference data.")
    if neuron_col not in data_gen.columns:
        raise ValueError(f"Column '{neuron_col}' not found in generalize data.")
    if set(data_ref[neuron_col].unique()) != set(data_gen[neuron_col].unique()):
        raise ValueError("Reference and generalize data must have the same neurons.")
    ...
    cond_grp_1, cond_grp_2 = [
        v[condition_col].unique().tolist()
        for v in group_df_by(data_ref, variable_col).values()
    ]

    # pre-processing
    if min_trials_ref is None:
        min_trials_ref = (
            data_ref.groupby(variable_col)
            .value_counts([neuron_col])
            .groupby(neuron_col)
            .min()
            .min()
        )
    if min_trials_gen is None:
        min_trials_gen = (
            data_gen.groupby(variable_col)
            .value_counts([neuron_col])
            .groupby(neuron_col)
            .min()
            .min()
        )
    data_ref = group_df_by(data_ref, neuron_col)
    data_gen = group_df_by(data_gen, neuron_col)
...
def decode_ref_to_gen(
    ...
    neuron_col : str
        Column name of neuron identities in data.
    min_trials_ref : int or None, default=None
        Minimum number of trials to include in each pseudo-population for reference data.
    min_trials_gen : int or None, default=None
        Minimum number of trials to include in each pseudo-population for generalize data.
    n_samples : int, default=250
        Number of neuron and trial samples to construct.
    subsample_ratio : float, default=0.75
        Ratio of neurons to include in pseudo-population for each sample construction.
